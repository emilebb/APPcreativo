      ...
      <div className="max-w-4xl w-full">
        ...
        {step === 'welcome' && (
          <div className="text-center space-y-8 animate-in fade-in duration-500">
            <div className="inline-flex items-center gap-3 bg-white/5 backdrop-blur-md rounded-2xl px-6 py-4 border border-white/10">
              ...
              <div className="text-left">
                <div className="text-2xl font-bold text-white">CreativoX AI</div>
                <div className="text-sm text-white/60">Coach Creativo Inteligente</div>
              </div>
            </div>
            ...
            <div className="max-w-md mx-auto space-y-4">
              <input
                ...
                className="w-full px-6 py-4 text-lg bg-white/5 border-2 border-white/10 rounded-xl text-white placeholder:text-white/40 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition"
                onKeyPress={(e) => e.key === 'Enter' && userName && setStep('goal')}
              />
              <button
                onClick={() => userName && setStep('goal')}
                disabled={!userName}
                className="w-full px-8 py-4 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-xl hover:from-violet-700 hover:to-indigo-700 transition-all font-semibold text-lg shadow-lg shadow-violet-500/30 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Comenzar
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {goals.map((goal) => (
                <button
                  ...
                  className="group p-6 bg-white/5 backdrop-blur-sm rounded-2xl border-2 border-white/10 hover:border-violet-500 hover:bg-white/10 transition-all text-left hover:scale-105"
                >
            ...
            <div className="grid gap-4">
              {blocks.map((block) => (
                <button
                  ...
                  className="group p-6 bg-white/5 backdrop-blur-sm rounded-2xl border-2 border-white/10 hover:border-violet-500 hover:bg-white/10 transition-all text-left hover:scale-102 flex items-start gap-4"
                >
                  ...
                  <ArrowRight className="w-5 h-5 text-white/40 group-hover:text-violet-400 transition" />
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 'processing' && (
          <div className="text-center space-y-8 animate-in fade-in duration-500">
            <div className="w-20 h-20 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-full flex items-center justify-center mx-auto animate-pulse">
            ...
            <div className="max-w-md mx-auto bg-violet-600/20 border border-violet-500/30 rounded-xl p-4">
